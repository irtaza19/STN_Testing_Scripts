...
WAIT_AFTER_ATZ       = 4.0      # reset usually prints banner; give it longer
WAIT_AFTER_WAKE_UART = 2.0      # pause after wake bytes to let prompt/banner arrive


# Use a pseudo step for wake
WAKE_STEP = "__WAKE_UART__"

# ===== SEQUENCE (exactly as requested; note spaces vs no spaces) =====
SEQUENCE = [
    ("STRSTNVM",  WAIT_AFTER_STRSTNVM),
    ("STSLEEP",          BASE_WAIT_SEC),
    (WAKE_STEP,          WAIT_AFTER_WAKE_UART),  # send wake bytes on UART
    ("STSLCS",           BASE_WAIT_SEC),
    ("ATPP 0E SV A8",    BASE_WAIT_SEC),
    ("ATZ",              WAIT_AFTER_ATZ),
    ("STSLCS",           BASE_WAIT_SEC),
    ("STRSTNVM",  WAIT_AFTER_STRSTNVM)
]
